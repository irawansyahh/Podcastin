...
import 'package:Podcastin/services/audio/audio_background_player.dart';
...
class MobileAudioPlayerService extends AudioPlayerService {
  ...
  final PublishSubject<PositionState> _playPosition =
      PublishSubject<PositionState>();
  ...
  Future<void> playEpisode(
      {@required Episode episode, bool resume = true}) async {
    if (episode.guid != '') {
      ...
      var currentState = AudioService.playbackState?.processingState ??
  ...
  Future<void> seek({int position}) async {
    ...
    _playPosition.add(PositionState(
        seconds, Duration(seconds: _episode.duration), complete.toInt()));
  ...
  Future<void> resume() async {
    ...
    if (_episode != null) {
      ...
      if (basicState == AudioProcessingState.none) {
        // Get the current playing state Podcastin thinks we're in
  ...
  Future<void> _onUpdatePosition() async {
    ...
    if (playbackState != null) {
      ...
      if (duration > 0) {
        ...
        var complete =
            position.inSeconds > 0 ? (duration / position.inSeconds) * 100 : 0;

        _playPosition.add(PositionState(
            position, Duration(seconds: _episode.duration), complete.toInt()));
      }
    }
  }
  ...
  Future<void> _start() async {
    ...
    await AudioService.start(
      ...
      androidNotificationChannelName: 'Podcastin Podcast Player',
  ...
  /// functions that Podcastin needs to run as the audio state changes. Ideally
  ...
  /// to ensure the necessary Podcastin is code is run upon state change.
  ...
  Future<void> _savePosition() async {
    ...
    if (_episode != null && _episode.downloaded) {
      ...
      log.fine(
          'Saving position for episode ${_episode.title} - ${_episode.position}');
