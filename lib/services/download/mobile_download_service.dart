...
class MobileDownloadService extends DownloadService {
  ...
  MobileDownloadService({@required this.repository})
  ...
  Future<bool> downloadEpisode(Episode episode) async {
    if (await hasStoragePermission()) {
      ...
      var filename = safePath(uri.pathSegments.firstWhere(
          (e) => e.toLowerCase().endsWith('.mp3'),
          orElse: () => null));

      filename ??= safePath(uri.pathSegments.firstWhere(
      ...
      if (filename == null) {
        ...
        log.fine(
            'Download episode (${episode?.title}) $filename to $downloadPath');
